...
def filter_spikes_by_region_stimulus_across_images(multi_probe_expt_info, region, stimulus, short_run=False):
    spike_trains = {}
        ...
        for c_probe in np.unique(data_set.unit_df['probe']):
            ...
            for index, region_unit in region_units.iterrows():
                ...
                for ind, stim_row in data_set.stim_tables['natural_scenes'].iterrows():
                    ...
                    train_id = multi_probe_filename + '_' + c_probe + '_' + region_unit['unit_id'] + '_' + str(region_unit['depth'])
