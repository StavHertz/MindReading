...
for region in all_regions:
    if region in get_all_crotical_regions():
        ...
        for index, row in region_units.iterrows():
            if not np.isnan(row['latency_sdf']):
                depth_thresh = cortex_depth[row['experiment'] + '_' + row['probe']]
                # print(row['depth'])
                # print(depth_thresh)
                # print('--')
                if int(row['depth']) > depth_thresh:
                    all_top_latencies.append(row['latency_sdf'])
                else:
                    all_bottom_latencies.append(row['latency_sdf'])
